...
class platesHome(ListView):
    ...
    context_object_name = 'movie'
    queryset = models.Product.objects.order_by('-id').reverse()
class penHome(ListView):
    template_name = 'pen.html'
    model = models.Product
    
    context_object_name = 'movie'
    queryset = models.Product.objects.order_by('-id').reverse()
class brushHome(ListView):
    template_name = 'brush.html'
    model = models.Product
    
    context_object_name = 'movie'
    queryset = models.Product.objects.order_by('-id').reverse()
class towelHome(ListView):
    template_name = 'towel.html'
    model = models.Product
...
def ReservationView(request):
    ...
    if request.method == 'POST':
        ...
        if ticket_form.is_valid():
            ...
            ticket.hq = request.user.userprofile.hq
            ticket.occupation = request.user.userprofile.occupation
            ticket.save()
            booked = True
        else:
...
def export(request):
    ...
    writer.writerow(['Product Name','Customer Name','Customer Age','Gender','Product Column','Ordered On','income','Highest Education','occupation'])


    for i in models.Ticket.objects.all().values_list('movie__name','user','age','gender','movie__column_number','post_date','income','hq','occupation'):
        writer.writerow(i)
